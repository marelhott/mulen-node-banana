...
export function Header() {
  ...
  const handleLogoClick = () => {
    // Pokud je canvas prázdný, jen zavřeme a znovu otevřeme quickstart
    if (!workflowName && !workflowId) {
      setShowQuickstart(true);
      return;
    }

    // Pokud existuje projekt, požádáme o potvrzení
    const message = hasUnsavedChanges
      ? 'Vrátit se na úvodní obrazovku? Neuložené změny budou ztraceny.'
            ...
            {isProjectConfigured ? (
              <>
                ...
                <button
                  ...
                  className="p-1 text-neutral-400 hover:text-neutral-200 transition-colors"
                  title="Nastavení projektu (změna cesty ukládání, jména, atd.)"
                >
                  <svg
                    className="w-4 h-4"
                    fill="none"
